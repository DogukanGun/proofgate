...
.venv/bin/python -m pytest -q        # 46 passing, no network needed

cp .env.example .env                 # add HEDERA_OPERATOR_ID / _KEY from portal.hedera.com
.venv/bin/python scripts/setup_treasury.py   # creates the payee -> put PROOFGATE_PAY_TO in .env

.venv/bin/python -m proofgate.gateway        # terminal 1: the gateway (holds NO key)
.venv/bin/python -m proofgate.demo.payer     # terminal 2: honest client: 402, real HBAR transfer, 200
...
|---|---|---|---|
| 1 | fabricate: tx never existed, valid window closed | no tx exists | **409 FRAUD P1** |
| 2 | pending: tx unknown, window still open | no tx exists | **402 HOLD P1** (no accusation) |
| 3 | wrong memo: real full-price transfer, bound to nothing | [real transfer] | **409 FRAUD P3** |
| 4 | short-pay: correct memo, 10% of the invoice | [real transfer] | **409 FRAUD P2** |
| 5 | double-spend: one honest payment, presented twice | [real transfer] | **200 GRANT, then 409 FRAUD P5** |

HTTP semantics: **402** = pay (or HOLD, where retry is meaningful), **409** = FRAUD (retry is pointless; the response carries a signed, offline-replayable attestation, also listed at `/attestations`).

## Live on-chain evidence (testnet, 2026-07-31)

Every transaction below was produced by the demos in this repo and verified by the gateway against the public mirror node. Payer [`0.0.9830134`](https://hashscan.io/testnet/account/0.0.9830134) paid treasury [`0.0.9861682`](https://hashscan.io/testnet/account/0.0.9861682), created by `scripts/setup_treasury.py`.

| What | Verdict | HashScan |
|---|---|---|
| honest payment, memo-bound, full price | **200 GRANT** | [`532496-239943027`](https://hashscan.io/testnet/transaction/0.0.9830134-1785532496-239943027) |
| real transfer, full price, wrong memo | **409 FRAUD P3** | [`532513-656908035`](https://hashscan.io/testnet/transaction/0.0.9830134-1785532513-656908035) |
| real transfer, correct memo, 10% of invoice | **409 FRAUD P2** | [`532518-726190090`](https://hashscan.io/testnet/transaction/0.0.9830134-1785532518-726190090) |
| honest payment presented twice | **200, then 409 FRAUD P5** | [`532523-924815177`](https://hashscan.io/testnet/transaction/0.0.9830134-1785532523-924815177) |

Open any of them: the memo field on HashScan shows the 43-character binding (or the wrong one), and the transfer list shows the exact tinybar amounts. The verdicts are re-derivable by anyone from the chain alone.
...
Hedera's mirror nodes expose a **free, keyless, public REST API** over full transaction history. No API key, no credits, no node to run, just a plain `GET`. Combined with ~3s deterministic finality and no reorg window, a gateway can independently confirm every settlement before it serves anything, at zero marginal cost.
...
### Independence: mirror x consensus node
...
> **On the 2-of-N mirror quorum:** the research this is based on assumes several independent mirror operators. We checked. On testnet, exactly **one** free keyless mirror endpoint exists: Hashio is a JSON-RPC relay (405 on REST), Arkhia and Validation Cloud require API keys. A free operator-diverse mirror quorum is **not currently buildable**, so we did not claim one. Mirror x consensus-node is the honest substitute, and arguably a stronger axis.

## Provenance, and what is actually ours

The design (the predicate ladder, the memo binding, the GRANT/FRAUD/HOLD split) comes from a paper we published: **[Don't Trust the Facilitator (10.5281/zenodo.21704301)](https://zenodo.org/record/21704301)** ([PDF in this repo](research/facilitator-lies-mirror-node-audit.pdf)).
...
**This repository is that design built against the real network.** None of the paper's quantitative claims are reproduced here, and none should be quoted as if this repo measured them. What we measure, we measure. Where the paper turned out to be wrong about Hedera, we say so:
...
**46 tests passing**, offline: the ladder runs against a scripted mirror, so every predicate (including "mirror outage must be HOLD, never FRAUD") is pinned without touching the network.
